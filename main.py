...
@register("group_summary_danfong", "Danfong", "群聊总结增强版", "0.1.40")
class GroupSummaryPlugin(Star):
    def __init__(self, context: Context, config: dict = None):
        ...
        logger.info(f"=== 群聊总结 {VERSION} 已加载 ===")
    ...
    async def html_render(self, template: str, data: dict, options: dict = None) -> Optional[str]:
        try:
            ...
            if "content-encoding: br" in err_str.lower():
                logger.error(f"[{VERSION}] 渲染失败: 缺少 brotli 依赖。请在插件目录执行: pip install brotli")
            else:
    ...
    async def summarize_group(self, event: AstrMessageEvent, payload=None, **kwargs):
        """
        手动指令：/总结群聊
        FIX: 兼容性参数 payload=None, **kwargs 防止参数报错
        """
        try:
            ...
            if img:
                ...
                yield event.plain_result(f"❌ 结果为空 (请检查后台日志是否有 Render Error)")
